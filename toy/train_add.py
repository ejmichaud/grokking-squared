...
def train_add(
    ...
    if batch_size > train_num:
        ...
        print(
            "batch size larger than the training set. We have set batch size=training size."
        )
    ...
    for i in P0D_c_id:
        ...
        for j in P0D_id_aug:
            P0D_aug.append(P0[j])
        null_dim_aug = np.sum(
            np.linalg.eigh(np.matmul(np.transpose(mat_aug), mat_aug))[0] < 1e-8
        )
    ...
    print(
        "acc_ideal_test = {}/{}={}".format(
            len(Dbar_id) - len(train_id), len(test_id), acc_ideal_test
        )
    )
    print(
        "the degree of freedom (except translation/scaling) for the reprsentation is {}".format(
            null_dim - 2
        )
    )
    print(
    ...
    reprs = torch.nn.parameter.Parameter(
        (torch.rand(p, reprs_dim) - 1 / 2).to(device) * init_scale_reprs
    )
    ...
    optimizer1 = torch.optim.AdamW(
        {reprs}, lr=eta_reprs, weight_decay=weight_decay_reprs
    )
    ...
    for step in range(steps):  # loop over the dataset multiple times
        ...
        if loss_type == "MSE":
            ...
            loss_train = nn.CrossEntropyLoss()(
                outputs_train, torch.tensor(out_id_train[choice], dtype=torch.long)
            )
            loss_test = nn.CrossEntropyLoss()(
                outputs_test, torch.tensor(out_id_test, dtype=torch.long)
            )
        ...
        if loss_type == "MSE":
            pred_train_id = torch.argmin(
                torch.sum(
                    (outputs_train.unsqueeze(dim=1) - y_templates.unsqueeze(dim=0))
                    ** 2,
                    dim=2,
                ),
                dim=1,
        ...
        reprs_scale = (reprs - reprs.mean(0).unsqueeze(dim=0)) / reprs.std(0).unsqueeze(
            dim=0
        )
        ...
        dists = reprs_scale[parallelograms[:, :2]].sum(axis=1) - reprs_scale[
            parallelograms[:, 2:]
        ].sum(axis=1)
    ...
    print(
        "final train acc=%.4f, test acc=%.4f, RQI=%.4f"
        % (acc_nn_train, acc_nn_test, rqi)
    )
    print(
        "Steps to reach thresholds: train acc={}, test acc={}, RQI={}".format(
            iter_train, iter_test, iter_rqi
        )
    )
    ...
    dic["pred_acc"] = (
        (all_num - train_num) * acc_pred_test + train_num * 1.0
    ) / all_num
    dic["dof"] = null_dim
